...
document.addEventListener('DOMContentLoaded', function () {
        ...
        try {
            ...
            usuarios.forEach(usuario => {
                ...
                const initials = usuario.full_name 
                    ? usuario.full_name.split(' ').map(n => n[0]).join('').toUpperCase().substring(0, 2)
                    : 'U';
                ...
                card.innerHTML = `
                    <div class="user-header">
                        <div class="user-avatar-admin">
                            ${usuario.avatar_url 
                                ? `<img src="${usuario.avatar_url}" alt="Avatar" class="user-avatar-img" />` 
                    ...
                    <div class="user-actions">
                        <button class="admin-card-btn btn-details view-user-reservations" 
                            data-id="${usuario.id}" 
                            data-name="${usuario.full_name || 'Usuario'}" 
                            data-email="${usuario.email}" 
                            data-initial="${initials}"
                            data-avatar="${usuario.avatar_url || ''}">
            ...
            document.querySelectorAll('.view-user-reservations').forEach(btn => {
                btn.addEventListener('click', async (e) => {
                    ...
                    const userAvatarUrl = btn.dataset.avatar;
                    ...
                    if (userAvatarUrl) {
                        userModalAvatar.innerHTML = `<img src="${userAvatarUrl}" alt="Avatar" class="user-avatar-img" />`;
                    } else {
    ...
    const userModalAvatarImg = document.getElementById('user-modal-avatar-img');
    const userModalAvatarFallback = document.getElementById('user-modal-avatar-fallback');
    ...
    // Función para obtener la URL del avatar desde Supabase
    ...
    // Función para actualizar un elemento de avatar
    async function updateAvatarElement(userId, imgElement, fallbackElement) {
        ...
        if (avatarUrl) {
            ...
            // Obtener nombre para las iniciales
            const { data: userData } = await supabase.auth.admin.getUserById(userId);
            if (userData && userData.user) {
                const fullName = userData.user.user_metadata?.full_name;
                fallbackElement.textContent = getInitials(fullName);
            } else {
    ...
    // Función para actualizar todos los avatares
    async function updateAllAvatars() {
        if (!currentUser) return;
        
        // Actualizar avatar de usuario normal
        if (userAvatarImg && userAvatarFallback) {
            await updateAvatarElement(currentUser.id, userAvatarImg, userAvatarFallback);
        }
        
        // Actualizar avatar de administrador
        if (adminAvatarImg && adminAvatarFallback) {
            await updateAvatarElement(currentUser.id, adminAvatarImg, adminAvatarFallback);
        }
        
        // Actualizar avatar en el modal
        if (userModalAvatarImg && userModalAvatarFallback) {
            await updateAvatarElement(currentUser.id, userModalAvatarImg, userModalAvatarFallback);
        }
    }
    ...
    async function cargarHorariosReservados(fechaISO) {
        try {
            ...
            const { data, error } = await supabase
                ...
                .eq('fecha', fechaISO);
                // .eq('estado', 'confirmed'); // Solo considerar reservas confirmadas
    ...
    async function cargarReservasUsuario() {
        try {
            ...
            reservas.forEach(reserva => {
                ...
                } else if (reserva.estado === 'completed') {
                ...
                reservaEl.innerHTML = `
                    ...
                    <div class="reservation-actions">
                        <button class="btn btn-danger cancel-reservation-btn" data-id="${reserva.id}" ${reserva.estado === 'canceled' || reserva.estado === 'completed' ? 'disabled' : ''}>
    ...
    async function cargarReservasAdmin(filter = 'all', customDate = null) {
        try {
            ...
            if (filter === 'today') {
                ...
                query = query.eq('estado', 'completed');
            ...
            reservas.forEach(reserva => {
                ...
                const reservaDateTime = new Date(`${reserva.fecha}T${reserva.hora}:00`);
                const ahora = new Date();
                const reservaPasada = reservaDateTime < ahora;
                
                // Crear HTML para el footer de la tarjeta
                let cardFooterHTML = `
                ...
                if (reserva.estado !== 'canceled' && reserva.estado !== 'completed') {
                    cardFooterHTML += `
                ...
                // Botón para marcar como completada (solo para reservas confirmadas que ya pasaron)
                if (reserva.estado === 'confirmed' && reservaPasada) {
                    cardFooterHTML += `
                        <button class="admin-card-btn btn-complete" data-id="${reserva.id}">
                            <i class="fas fa-check-double"></i> Completada
                        </button>
                    `;
                }
                
                // Confirmar pago solo si no está completada o cancelada
                if (!reserva.pago_verificado && reserva.estado !== 'canceled' && reserva.estado !== 'completed') {
                    cardFooterHTML += `
                    ...
                    <div class="reservation-card-header">
                        ...
                        <span class="reservation-status status-${reserva.estado}">
                            ${reserva.estado === 'pending' ? 'Pendiente' : 
                              reserva.estado === 'confirmed' ? 'Confirmada' : 
                              reserva.estado === 'completed' ? 'Completada' : 'Cancelada'}
                        </span>
                    </div>
            ...
            document.querySelectorAll('.btn-complete').forEach(btn => {
                btn.addEventListener('click', async () => {
                    const reservaId = btn.dataset.id;
                    await marcarReservaCompletada(reservaId);
                });
            });
...
if (numeroWhatsApp) {
    ...
    // Función para marcar una reserva como completada (admin)
    async function marcarReservaCompletada(reservaId) {
        try {
            showLoader(true);
            ...
            showNotification('Reserva marcada como completada', false, false, true);
    ...
    async function generateAdminCalendar(month, year) {
        ...
        for (let day = 1; day <= lastDay.getDate(); day++) {
            ...
            if (reservasDia.includes('completed')) {
                const indicator = document.createElement('div');
                indicator.classList.add('event-indicator', 'event-completed');
                indicatorsContainer.appendChild(indicator);
            }
    ...
    // Función para eliminar reservas pasadas automáticamente (MODIFICADA)
    async function eliminarReservasPasadas() {
        try {
            const today = new Date();
            const todayISO = formatLocalDate(today);
            
            // Obtener reservas pendientes pasadas (excluyendo completadas)
            const { data: reservasPasadas, error: fetchError } = await supabase
                .from('reservas')
                .select('id, estado, fecha, hora')
                .lt('fecha', todayISO)
                .or('estado.eq.pending,estado.eq.confirmed');
            ...
            reservasPasadas.forEach(reserva => {
                ...
                // Solo eliminar pendientes expiradas, mantener confirmadas para marcarlas como completadas
                if (reserva.estado === 'pending' && reservaDateTime < ahora) {
                    reservasAEliminar.push(reserva.id);
                }
            });
            ...
            if (reservasAEliminar.length > 0) {
                const { error: deleteError } = await supabase
                    .from('reservas')
                    .delete()
                    .in('id', reservasAEliminar);
                
                if (deleteError) throw deleteError;
                
                console.log(`Se eliminaron ${reservasAEliminar.length} reservas pendientes expiradas`);
            }
            ...
            console.error('Error procesando reservas pasadas:', error);
        }
    }
